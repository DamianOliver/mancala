    ...
    def calculate(self, depth, current_index, calc_board, alpha, beta):
        ...
        for i in range(6):
            if calc_board[0][i + index_offset] != 0:
                ...
                # draw(test_board)
                evals.append(self.calculate(depth + 1, current_index * index_mult, test_board, alpha, beta)[0])
                top_line_list.append(self.calculate(depth + 1, current_index * index_mult, test_board, alpha, beta)[1])
            else:
                blanks += 1
                if current_index == 1:
                    evals.append(-1000)
                    top_line_list.append([])
                else:
                    evals.append(1000)
                    top_line_list.append([])
            if current_index == 1:
                if max(evals) >= beta:
                    top_line_list.append(7)
                    return (max(evals)), top_line_list
                ...
                if min(evals) <= alpha:
                    top_line_list.append(7)
                    return (min(evals)), top_line_list
        ...
        if depth == 1:
            ...
            return best_move, top_line

        return best_eval, top_line
